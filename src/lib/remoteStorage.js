...
export default {
	...
	async addSshRemote(...args) {
		...
		loader.create(strings["add ssh remote"], strings["connecting..."], {
			timeout: 10000,
			callback() {
				stopConnection = true;
			},
		...
		// SSH Remote uses SFTP for file operations
		const connection = Sftp(hostname, Number.parseInt(port), username, {
			password,
			keyFile,
			passPhrase,
		...
		try {
			...
			loader.destroy();
			await helpers.error(err);
			return await this.addSshRemote(
				hostname,
				username,
				keyFile,
				password,
		...
		function prompt(
			...
			return multiPrompt(strings["add ssh remote"], inputs);
		}
	},
